...
const loadData = async (subcategory) => {
  try {
    const response = await fetch(`/data/etaireia-psychikon-ereynon/${subcategory}.json`);
...
const Εtaireia = () => {
  const [selectedSub, setSelectedSub] = useState('erevnes-fainomena');
        ...
        {error ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {articles.map(article => (
              ...
              >
                <Link
                  to={`/etaireia-psychikon-ereynon/${selectedSub}/${article.slug}`}
                  className="block"
                >
                  {article.image?.src && (
                <img
