...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const data = await getEfimeridesArticle(subcategory, slug, locale);
  ...
  const canonical =
    seo.canonical ??
    `/efimerides/${subcategoryData.subcategorySlug ?? subcategoryData.slug}/${article.slug}`;
...
export default async function EfimeridesArticlePage({ params }: PageProps) {
  const { subcategory, slug } = await params;
  const locale = await getRequestLocale();
  const data = await getEfimeridesArticle(subcategory, slug, locale);
  ...
  const canonicalPath = `/efimerides/${subcategoryData.subcategorySlug ?? subcategoryData.slug}/${article.slug}`;
